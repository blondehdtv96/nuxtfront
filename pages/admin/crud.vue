<template>
    <v-container grid-list-xs>
        <v-text-field v-model="databaru.nama" name="name" label="nama" ></v-text-field>
        <v-text-field v-model="databaru.id" name="id" label="id" ></v-text-field>
        <v-text-field v-model="databaru.umur" name="umur" label="label" ></v-text-field>
        <v-btn color="success" @click="tambahkaryawan">tambah</v-btn>
        <v-btn color="success" @click="tambahkaryawanid">tambah pake id</v-btn>

        <v-divider></v-divider>

        <v-data-table :headers="headers" :items="karyawan" v-if="karyawan.length > 0">
...
export default {
    ...
    methods: {
       async tambahkaryawanid(){
        try {
                await db.collection('karyawan').doc(this.databaru.id).set({ nama: this.databaru.nama, umur: this.databaru.umur })
                this.tarikdatakaryawan()
        ...
        async tarikdatakaryawan() {
            this.karyawan = []
            return await db.collection('karyawan').get().then((doc) => {
                if (doc.size > 0) {
                    doc.forEach(doc2 => {
        ...
        async tambahkaryawan() {
            try {
                await db.collection('karyawan').add({ nama: this.databaru.nama, umur: this.databaru.umur })
                this.tarikdatakaryawan()
                this.databaru = { nama: "", umur: 0 }
            } catch (error) {
                console.log(error)
